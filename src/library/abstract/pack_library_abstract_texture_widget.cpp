#include "pack_library_abstract_texture_widget.hpp"
...
texture_widget::texture_widget(const std::uint32_t c_width,
    ...
    : widget{c_width, c_height, c_title},
      mc_scene{c_scene},
...
texture_widget::~texture_widget() noexcept { PACK_LIBRARY_LOG_FUNCTION_CALL(); }

const sf::View& texture_widget::get_view() const noexcept {
  PACK_LIBRARY_LOG_FUNCTION_CALL();

  return this->m_view;
}

void texture_widget::set_view(const sf::View& c_view) noexcept {
  PACK_LIBRARY_LOG_FUNCTION_CALL();

  this->m_view = c_view;
}

void texture_widget::fill_image_fields() noexcept {
  PACK_LIBRARY_LOG_FUNCTION_CALL();
